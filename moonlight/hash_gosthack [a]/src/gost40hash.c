...
void gost40hash_compress(unsigned long *h, unsigned long *m) {
  /* here goes the cheat */
  int i;
  unsigned long h2[8] = {0, 0, 0, 0, 0, 0, 0, 0};
  h2[0] = h[0];
  h2[1] = h[1];
  gosthash_compress(h2, m);
  h[0] = h2[0];
  h[1] = h2[1] & 0x000000FF;
}
...
void gost40hash_bytes(GostHashCtx *ctx, const unsigned char *buf, size_t bits)
{
  int i, j;
  ...
  gost40hash_compress(ctx->hash, m);
...
void gost40hash_update(GostHashCtx *ctx, const unsigned char *buf, size_t len)
{
  ...
  gost40hash_bytes(ctx, ctx->partial, 256);

  while ((j + 32) < len)
  {
    gost40hash_bytes(ctx, &buf[j], 256);
    j += 32;
  }
...
void gost40hash_final(GostHashCtx *ctx, unsigned char *digest)
{
  ...
  if (ctx->partial_bytes > 0)
  {
    ...
    gost40hash_bytes(ctx, ctx->partial, ctx->partial_bytes << 3);      
  }

  gost40hash_compress(ctx->hash, ctx->len);  
  /* another cheat - we don't mix the sum in */
  //gost40hash_compress(ctx->hash, ctx->sum);  
